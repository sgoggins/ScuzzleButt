...
// TwitterZombie - collects tweets using the twitter search API and stores
...
$db_connection = mysql_connect('tzn-drexel-2.cbqyy1dvl5yb.us-east-1.rds.amazonaws.com','skippy','happyplug');
if (!$db_connection) {
...
$db_select_result = mysql_select_db('tzn2');
if (!$db_select_result) {
...
while ($row = mysql_fetch_assoc($job_select_result)) {
	...
	while(!empty($decode["results"])){
		...
		//print_r($result);
			...
			// Check for geo data of type point
			$my_geo = "''";
			//print_r($result);
			...
			$tweet_update_query = "INSERT INTO tweet (tweet_id_str, job_id, created_at, text, from_user, from_user_id_str, from_user_name, to_user, to_user_id_str, to_user_name, source, iso_language, location_geo)"
